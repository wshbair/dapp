...
const bytecode = output.contracts[':Eethiq'].bytecode;
const abi = JSON.parse(output.contracts[':Eethiq'].interface);
  ...
  function (e, contract) {
    ...
    if (typeof contract.address !== 'undefined') {
      console.log("contract mined: " + contract.address);     
            ...
            console.log("----------------------------------");
            contract.newCharity("IFPS", {gas: 2100000, from: web3.eth.accounts[0]});
            contract.newProject("Fooding", 1, 100000, 1, {gas: 2100000, from: web3.eth.accounts[0]});
            
            var p = poissonProcess.create(10000, function NewDonor(){
                var donneraddress="0x"+Sha1.hash("Wazen"+new Date().getTime()); 
                clientaddresses.push(donneraddress);
                var SenderName='testingWazen';
                var txhash1= contract.newDonor(SenderName,donneraddress, {gas: 2100000, from: web3.eth.accounts[0]});
                console.log('New Donor sent : '+ donneraddress);
...
//======================================================================================================================
 function CheckRegistationStatus(clientaddress,contract) {
		var clientinfo=contract.getDonor.call(clientaddress, {gas: 2000000, from: web3.eth.accounts[0]});
		if(web3.toUtf8(clientinfo[0].toString()).length>2)
		 {
             console.log(web3.toUtf8(clientinfo[0].toString()));
             return true;
		 }
         else
         return false;
